"""Tests for the W4 ToolProfile dispatch in porkbun-dns-mcp.
...
from porkbun_dns_mcp import tools as tools_pkg
from porkbun_dns_mcp.config import PorkbunDNSSettings
from porkbun_dns_mcp.server import create_app
from porkbun_dns_mcp.tools.profiles import (
    _GROUP_REGISTRY,
    FULL_REGISTRATIONS,
    MINIMAL_REGISTRATIONS,
    PROFILE_REGISTRATIONS,
    _build_registration_map,
    apply_porkbun_dns_tool_profile,
...
def _test_settings(**overrides: Any) -> PorkbunDNSSettings:
    defaults: dict[str, Any] = {
        "api_key": "test-key",
        "secret_key": "test-secret",
    }
    defaults.update(overrides)
    return PorkbunDNSSettings.model_validate(defaults)
...
def test_profiles_py_exists() -> None:
    """``porkbun_dns_mcp/tools/profiles.py`` must exist."""
    import porkbun_dns_mcp.tools.profiles  # noqa: F401
...
def test_profiles_py_defines_group_registry() -> None:
    ...
    expected = {
        ...
        ("dns_tools", "register_dns_tools_for_profile"),
    }
...
def test_profiles_py_defines_apply_porkbun_dns_tool_profile() -> None:
    """``apply_porkbun_dns_tool_profile`` is async (returns a coroutine fn)."""
    assert callable(apply_porkbun_dns_tool_profile)
    import asyncio

    assert asyncio.iscoroutinefunction(apply_porkbun_dns_tool_profile)


def test_profiles_py_references_correct_env_var() -> None:
    """The dispatch helper is called with ``PORKBUN_DNS_TOOL_PROFILE``."""
    import inspect

    source = inspect.getsource(apply_porkbun_dns_tool_profile)
    assert "PORKBUN_DNS_TOOL_PROFILE" in source
...
def test_register_dns_tools_for_profile_exists_in_tools_package() -> None:
    """``register_dns_tools_for_profile`` is exposed at the tools package level."""
    assert hasattr(tools_pkg, "register_dns_tools_for_profile")
    assert callable(tools_pkg.register_dns_tools_for_profile)
...
def test_profiles_uses_async_helper_not_sync_wrapper() -> None:
    """``apply_porkbun_dns_tool_profile`` calls ``_apply_tool_profile``,
...
def test_server_awaits_apply_porkbun_dns_tool_profile() -> None:
    """AST guard: ``server.py`` MUST ``await apply_porkbun_dns_tool_profile``.

    Structural check for ``ast.Await(value=ast.Call(func=ast.Name(
    id='apply_porkbun_dns_tool_profile')))``. NOT just a count of calls
    (the W3.2 round-1 lesson — count-only guards produce false
    positives that silently break in event loops).
    """
    from pathlib import Path

    server_path = Path(__file__).resolve().parents[2] / "porkbun_dns_mcp" / "server.py"
    ...
    def _walk(node: ast.AST) -> bool:
        if isinstance(node, ast.Await) and isinstance(node.value, ast.Call):
            ...
            if (
                ...
                and func.id == "apply_porkbun_dns_tool_profile"
            ):
                return True
    ...
    assert _walk(tree), (
        "server.py must await apply_porkbun_dns_tool_profile (the W2b.3 "
...
def test_server_does_not_call_sync_apply_tool_profile() -> None:
    ...
    assert "apply_tool_profile(" not in stripped, (
        ...
        "_apply_tool_profile via apply_porkbun_dns_tool_profile)."
    )
...
def test_guard_fails_when_await_is_removed() -> None:
    ...
    Build a synthetic module that calls ``apply_porkbun_dns_tool_profile``
    ...
    source = """
async def bad():
    apply_porkbun_dns_tool_profile(server, settings, client)
"""
...
def test_caller_supplied_settings_are_preserved(
    ...
    """Monkey-patch ``PorkbunDNSSettings.__init__`` so any env reload raises.
    ...
    original_init = PorkbunDNSSettings.__init__

    def tracking_init(self: PorkbunDNSSettings, *args: Any, **kwargs: Any) -> None:
        init_call_count["n"] += 1
        original_init(self, *args, **kwargs)

    monkeypatch.setattr(PorkbunDNSSettings, "__init__", tracking_init)
    settings = _test_settings()
...
async def test_lifespan_actually_calls_client_close_on_shutdown() -> None:
    ...
    server, monkey-patch ``PorkbunClient.close``, and verify the close
    ...
    original_close = PorkbunClient.close

    async def tracking_close(self: PorkbunClient) -> None:
        close_called["n"] += 1
        await original_close(self)

    with patch.object(PorkbunClient, "close", tracking_close):
...
def test_essential_tool_names_subset_check_enforced() -> None:
    """Source-level: ``apply_porkbun_dns_tool_profile`` passes
...
async def test_minimal_profile_registers_only_health_check() -> None:
    ...
    with patch.dict("os.environ", {"PORKBUN_DNS_TOOL_PROFILE": "minimal"}):
        await apply_porkbun_dns_tool_profile(
...
async def test_standard_profile_registers_all_groups() -> None:
    ...
    settings = _test_settings()
    server = _fresh_server()
    with patch.dict("os.environ", {"PORKBUN_DNS_TOOL_PROFILE": "standard"}):
        await apply_porkbun_dns_tool_profile(
            server, settings, client=object()  # type: ignore[arg-type]
        )
    names = {tool.name for tool in await server.list_tools()}
...
async def test_full_profile_registers_all_groups() -> None:
    ...
    settings = _test_settings()
    server = _fresh_server()
    with patch.dict("os.environ", {"PORKBUN_DNS_TOOL_PROFILE": "full"}):
        await apply_porkbun_dns_tool_profile(
            server, settings, client=object()  # type: ignore[arg-type]
        )
    names = {tool.name for tool in await server.list_tools()}
...
async def test_create_app_invalid_profile_raises() -> None:
    """A SET-BUT-INVALID ``PORKBUN_DNS_TOOL_PROFILE`` raises
    ``InvalidProfileError`` (per the W0 spec — UNSET falls through to
    FULL, but a bogus value is loud)."""
    settings = _test_settings()
    server = _fresh_server()
    with patch.dict("os.environ", {"PORKBUN_DNS_TOOL_PROFILE": "bogus-value"}):

"""MCP tool registration for porkbun-dns-mcp.
...
``PorkbunClient`` (the old sync ``create_app`` constructed the client
...
2. **Lifespan cleanup** — the ``PorkbunClient`` instance must be the
...
def register_health_tool(
    mcp: FastMCP, settings: PorkbunDNSSettings, client: PorkbunClient | None = None
) -> None:
    ...
    async def health_check() -> dict[str, Any]:
        ...
        Returns:
            ...
            and configured state (whether PORKBUN_DNS_API_KEY and
            PORKBUN_DNS_SECRET_KEY are set).
        """
        return {
            "status": "healthy",
...
def register_dns_tools_for_profile(
    mcp: FastMCP, settings: PorkbunDNSSettings, client: PorkbunClient
) -> None:
    """Profile-dispatch entry for the dns_tools group.

    Forwards to the legacy ``register_dns_tools`` (which takes a
    pre-constructed ``PorkbunClient``).
    """
    register_dns_tools(mcp, client)


# ---------------------------------------------------------------------------
# Backward-compat shim — pre-W4 callers (tests, examples) still call
# ``register_dns_tools(app, client)`` directly. Kept so this shim does
...
__all__ = [
    "register_dns_tools",
    "register_dns_tools_for_profile",

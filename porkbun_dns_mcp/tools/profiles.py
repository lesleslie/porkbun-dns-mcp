"""Tool profile registration groups for porkbun-dns-mcp MCP server.
...
``PORKBUN_DNS_TOOL_PROFILE`` environment variable.
...
Profile tiers (Tier-A trivial — 3-tier mapping per the W4 plan):
    ...
    STANDARD: All 5 porkbun-dns-mcp tools + ``health_check`` +
              ``discover_tools`` (Tier-A trivial: same set as FULL —
              there's no "core subset" to drop at STANDARD).
    FULL:     All 5 porkbun-dns-mcp tools + ``health_check`` +
...
``register_all_tool_groups`` + ``apply_porkbun_dns_tool_profile``) is
consumed by ``porkbun_dns_mcp.server.create_app`` which delegates to
...
# attribute name on the ``porkbun_dns_mcp.tools`` package. THIS is the
...
# ``getattr(porkbun_dns_mcp.tools, attr_name)`` — no name-specific
...
def _build_registration_map(
    settings: PorkbunDNSSettings, client: PorkbunClient
    ...
    ``porkbun_dns_mcp.tools`` package (no hard-coded name-specific
    ...
    Args:
        settings: The caller-supplied ``PorkbunDNSSettings`` instance to
        ...
        client: The pre-constructed ``PorkbunClient`` instance. Same
    ...
    Returns:
        Mapping from group key (e.g. ``"dns_tools"``) to a single-arg
        async-or-sync callable that takes the ``FastMCP`` server.
    """
    from porkbun_dns_mcp import tools as _tools_module
...
def register_all_tool_groups(
    server: FastMCP, settings: PorkbunDNSSettings, client: PorkbunClient
) -> None:
    """Bulk register every porkbun-dns-mcp tool group (called at FULL profile).

    Iterates ``_GROUP_REGISTRY`` directly — no name-specific
    conditionals. Each entry's ``attr_name`` is looked up dynamically on
    the ``porkbun_dns_mcp.tools`` package. Used as ``register_all_fn``
    for the W0 helper.

    Args:
        server: FastMCP server instance.
        settings: The caller-supplied ``PorkbunDNSSettings`` to pass
            through to every group registration (NOT re-loaded from env).
        client: The pre-constructed ``PorkbunClient`` to bind into
...
async def apply_porkbun_dns_tool_profile(
    server: FastMCP, settings: PorkbunDNSSettings, client: PorkbunClient
) -> None:
    """Apply the ``PORKBUN_DNS_TOOL_PROFILE`` dispatch to ``server`` at startup.

    Async because the W0 helper is async; called from
    ``porkbun_dns_mcp.server.create_app`` via
    ``await apply_porkbun_dns_tool_profile(server, settings, client)``.
    ...
    await _apply_tool_profile(
        server,
        profile_env_var="PORKBUN_DNS_TOOL_PROFILE",

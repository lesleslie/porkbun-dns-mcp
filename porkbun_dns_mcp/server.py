...
from porkbun_dns_mcp.tools.profiles import apply_porkbun_dns_tool_profile
...
async def create_app(
    settings: PorkbunDNSSettings | None = None,
    ...
    Args:
        settings: Optional pre-loaded ``PorkbunDNSSettings``. When
        ...
        server: Optional pre-constructed ``FastMCP`` server. When
            ...
            wired to the ``PorkbunClient`` instance. Tests inject a
    ...
    # Construct the PorkbunClient ONCE upfront. The W0 dispatch lambdas
    ...
    # Apply tool profile dispatch (PORKBUN_DNS_TOOL_PROFILE env var).
    #
    # Replaces the previous direct ``register_dns_tools(app, client)``
    # call. The W0 helper from mcp-common 0.18.0+ dispatches by group
    # name and always registers the ``discover_tools`` meta-tool. The
    # default (no env var) remains FULL = all 5 porkbun-dns-mcp tools

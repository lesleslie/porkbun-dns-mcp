"""Configuration for Porkbun DNS MCP server.
...
2. settings/porkbun-dns.yaml (committed, for production defaults)
3. settings/local.yaml (gitignored, for development)
4. Environment variables PORKBUN_DNS_{FIELD}
"""
...
class PorkbunDNSSettings(BaseSettings):
    """Settings for Porkbun DNS MCP server.

    Uses pydantic-settings for environment variable support with
    Oneiric-style layered configuration.

    Attributes:
        api_key: Porkbun API key (from PORKBUN_DNS_API_KEY env var)
        secret_key: Porkbun secret key (from PORKBUN_DNS_SECRET_KEY env var)
    ...
    def has_credentials(self) -> bool:
        ...
        Returns:
            True if both api_key and secret_key are set
        """
    ...
    def auth_payload(self) -> dict[str, str]:
        ...
        All Porkbun API requests require apikey and secretapikey
        in the request body.
    ...
    def http_client_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "headers": {
                ...
                "User-Agent": "porkbun-dns-mcp/0.1.1",
            },
        }

    def get_logging_config(self) -> dict[str, Any]:
        """Get Oneiric-compatible logging configuration.

        Returns:
            Dictionary with LoggingConfig-compatible settings
        """
        return {
            "level": self.log_level,
            "emit_json": self.log_json,
            "service_name": "porkbun-dns-mcp",
        }


def setup_logging(settings: PorkbunDNSSettings | None = None) -> None:
...
@lru_cache
def get_settings() -> PorkbunDNSSettings:
    """Get cached settings instance.

    Returns:
        PorkbunDNSSettings instance loaded from environment
    """
    return PorkbunDNSSettings()


def get_logger_instance(name: str = "porkbun-dns-mcp") -> Any:
    """Get a structured logger instance.

    Uses Oneiric's get_logger if available, otherwise falls back
    to standard logging.
